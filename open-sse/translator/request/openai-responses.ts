 ...
 *
 * Superseded for new call sites by collectResponsesCustomToolNames /
 * collectCustomToolNamesForSourceFormat in ./openai-responses/additionalTools.ts
 * (which also handles `namespace` tool groups), but chatCore.ts still calls this
 * directly for one code path, so it stays exported.
 */
...
export function openaiResponsesToOpenAIRequest(
  ...
  // declarations (exec composer, function tools, MCP `namespace` groups). Tools may
  // also arrive split across more than one such item. collectResponsesTools() merges
  // all of these sources (top-level + every additional_tools item, with namespace
  // dedup) so they flow through the same validation/conversion as top-level tools;
  // otherwise they would be silently dropped, the upstream would receive no
  // structured `tools` array, and a native tool-capable local model
  // (rapid-mlx --enable-auto-tool-choice) would never engage its tool-calling
  // grammar, so Codex would loop without ever making a real tool call.
  ...
  // Convert tools format (top-level `tools` merged with any `additional_tools`
  // items via collectResponsesTools() above).
  if (tools.length > 0) {
    result.tools = tools
      .filter((toolValue) => {
        const tool = toRecord(toolValue);
