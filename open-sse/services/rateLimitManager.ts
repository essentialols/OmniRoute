...
import { markLocalRateLimitError, RATE_LIMIT_QUEUE_WEDGED_CODE } from "./rateLimitManager/errors";
...
export async function withRateLimit(provider, connectionId, model, fn, signal = null) {
  ...
  // Issue #6593: opt-in admission cap. Fast-reject before Bottleneck's schedule()
  // (and before any downstream compression/prompt work runs) when the queue is
  // already at/over maxQueueDepth. Default 0 = disabled.
  ...
  try {
    if (signal) {
      ...
      // `controller.abort()` with no reason yields a DOMException whose `name`
      // is a getter (already "AbortError"), so assigning to it throws a
      // TypeError inside this listener and the abort never propagates. Only
      // name the errors we construct ourselves. Every branch also cancels the
      // job via abortJob: the wrapper rejecting must not leave fn() running
      // upstream as an orphan burning quota after the caller gave up.
      ...
      try {
        ...
        // one of the others wins, the job promise (still running inside
        // Bottleneck) must not surface an unhandledRejection once it eventually
        // settles; the no-op catch observes it without changing the raced result.
        const job = scheduleJob();
        job.catch(() => {});
        const racers = [job, abortPromise];
        if (queueWaitPromise) {
          queueWaitPromise.catch(() => {});
    ...
    // Defensive: `expiration` is no longer set on our own scheduleJob(), but a
    // limiter configured elsewhere could still surface Bottleneck's raw
