...
export class PerplexityWebExecutor extends BaseExecutor {
  ...
  async execute({ model, body, stream, credentials, signal, log, onCredentialsRefreshed }: ExecuteInput) {
    ...
    if (thinking && THINKING_MAP[model]) {
      // "copilot", not "search": the retired "search" mode now returns FAILED — the
      // backend downgrades it to CONCISE and drops model_preference, so the thinking
      // variant would fail the same way the catalog models do (see the note above
      // MODEL_MAP). The thinking model is selected via model_preference.
      pplxMode = "copilot";
