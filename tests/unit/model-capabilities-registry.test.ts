...
test("canonical model capability resolver lets exact synced metadata override global specs", () => {
  modelsDevSync.saveModelsDevCapabilities({
    ...
    // NOTE: `agy` is the advertised/canonical provider id at the registry level
    // (open-sse/config/providerRegistry.ts), but open-sse/services/model.ts's
    // ALIAS_TO_PROVIDER_ID still maps "agy" -> "antigravity" for routing/capability
    // resolution, so a synced-capability lookup for "agy/<model>" is actually keyed
    // by "antigravity" here. Save under "antigravity" to match what
    // getResolvedModelCapabilities actually looks up.
    antigravity: {
      // gemini-pro-agent is the callable High id; MODEL_SPECS["gemini-3.1-pro"].aliases
      // includes both "gemini-pro-agent" and "gemini-3.1-pro-high", so either query
      // model resolves to the shared Gemini 3.1 Pro capability family.
