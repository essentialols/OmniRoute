...
test("chat pipeline applies Codex CLI fingerprint to OAuth responses requests", async () => {
  ...
  // broke this assertion while the rest of the test kept passing. Platform/arch
  // stay MacOS/arm64: the local fork's DEFAULT_CODEX_USER_AGENT_PLATFORM/ARCH
  // (open-sse/config/codexClient.ts) are pinned to match the real machine this
  // fork's capture daemon runs on, not upstream's generic Windows default.
  assert.equal(
    call.headers["User-Agent"],
    `codex-cli/${getCodexClientVersion()} (MacOS 24.0.0; arm64)`
  );
