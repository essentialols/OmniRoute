...
async function postExchangeAntigravity(
  ...
  if (projectId) {
    ...
    // Accounts without an existing Cloud Code project need onboarding before
    // loadCodeAssist can discover their project. onboardUser is a Long-Running
    // Operation that does not always finish on the first call, so retry bounded
    // by ONBOARD_TOTAL_BUDGET_MS instead of declaring BYOP or giving up after a
    // single attempt (the original design here was a single inline shot; the
    // login is worthless without a project, and the bound preserves the
    // "never hang the OAuth login" guarantee that motivated backgrounding this
    // in the first place).
    let onboardedWithoutProject = false;
    const onboardDeadline = Date.now() + ONBOARD_TOTAL_BUDGET_MS;
    do {
      try {
        const response = await fetchFirstOk(
          config.onboardUserEndpoints,
      ...
      if (projectId || onboardedWithoutProject) break;
      await new Promise((resolve) => setTimeout(resolve, ONBOARD_POLL_INTERVAL_MS));
    } while (Date.now() < onboardDeadline);
    if (!projectId) {
      return {
        userInfo,
        projectId,
