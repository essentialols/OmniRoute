...
function resolveTerminalConnectionStatus(
  ...
  // Regression contract: tests/unit/auth-terminal-status.test.ts asserts a bare
  // 401 on a plain API-key provider (openai/apikey) stays recoverable, not just
  // cookie-auth providers.
...
export async function markAccountUnavailable(
  ...
  try {
    ...
    const rawCooldownMsWithQuotaReset = cachedQuotaResetMs
    ...
    const isIntentionalNoCooldownRecoverable =
      providerErrorType === PROVIDER_ERROR_TYPES.OAUTH_INVALID_TOKEN ||
      providerErrorType === PROVIDER_ERROR_TYPES.PROJECT_ROUTE_ERROR;
    let recoverableRawCooldownMs = rawCooldownMsWithQuotaReset;
    if (
      !terminalStatus &&
