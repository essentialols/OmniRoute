...
async function startMitmServer() {
  ...
  const server = https.createServer(sslOptions, async (req, res) => {
    ...
    if (!mappedOverride) {
      ...
      // The #8656 captureToInspector call above already recorded this request
      // (as "in-flight", with no update path back to that same row); skip
      // passthrough()'s own response-completing capture here so this branch
      // doesn't also insert a second, duplicate entry for the same request.
      return passthrough(req, res, bodyBuffer, false);
    }
